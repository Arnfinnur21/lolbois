...
export default function StatCard({ name, iconUrl, value, valueKey, size = "sm", idx }) {
  const sizeClasses = size === "lg" ? "p-4 text-lg w-40" : "p-2 text-sm w-28";
  const { ring, gradient, labelBadge } = RANK_CLASSES[idx] ?? { ring: "", gradient: "", labelBadge: "" };
  const values = formatStat(valueKey, value);

  return (
    <div className={`rounded-2xl p-[2px] ${gradient}`}>
      <div className={`rounded-2xl bg-gray-900/90 ${ring} flex flex-col items-center ${sizeClasses}`}>
        <div className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs mb-1 ${labelBadge}`}>
          {idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : "—"} {values.label ?? name}
        </div>
        <img src={iconUrl} alt={name} className="w-12 h-12 rounded-full mb-2" />
        <div className="font-bold text-center">{name}</div>
        <div className="text-yellow-400">{values.value}</div>
      </div>
    </div>
  );
}

...
export const STAT_DESCRIPTORS = {
	...
	favItem: {
		...
		subtitle: "Most Frequently Purchased Legendary",
		selector: "favItem",
		order: "max",
	},
};
...
export function rankBy(players, descriptor) {
	let sorted =
		descriptor.order === "max"
			? [...players].sort(
					(a, b) => b[descriptor.selector] - a[descriptor.selector]
			  )
			: descriptor.order === "min"
			? [...players].sort(
					(a, b) => a[descriptor.selector] - b[descriptor.selector]
			  )
			: "";
	return sorted;
}
...
export const findMostFrequent = (arr) => {
	const clean = (arr ?? []).filter(Number.isFinite);
	if (clean.length === 0) return null;

	const countMap = new Map();
	let mostFrequent = clean[0];
	let maxCount = 0;
...
export const getMasteryData = (mastery) => {
